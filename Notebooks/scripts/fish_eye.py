...
This script applies a fish-eye distortion to a whole dataset:
    - Warps all the images within a dataset (RGB, segmentation, depth, normals) and saves them as
      <original_name_fisheye.png/.exr> in the same folder.
    - Same for the key points - each JSON file has its fisheye version: <original_name_fisheye.json>
...
Optional arguments:
    -d - A list of the 4 distortion coefficients: k1, k2, k3, k4 (default is: [0.17149, -0.43181, 0.25787, -0.08054]) -
         According to OpenCV's fisheye module documentation
...
def create_pixels_maps(im_pixels_map, width, height, cam_intrinsic, dist_coeff):
    ...
    undistorted_pixel_map = np.flip(undistorted_pixel_map, axis=2)  # Since cv2 is height first --> (W, H ,2)
...
def create_distorted_jsons(dp_cam_path, dis_pixel_map, orig_pixel_map, x_crop, y_crop, ar_x, ar_y, visualize):
    all_dist_jsons = {}
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d',
                        ...
                        nargs='+',
                        default=[0.17149, -0.43181, 0.25787, -0.08054])
    ...
    FPS = 5
    CODEC = 'MJPG'
    VIDEO_FRAME_SIZE = (512, 512)  # Affects only the output video

    # Load the dataset
    try:
        dataset = dg.load(args.dataset_path)
    except ():
        print("Not a valid dataset directory! Dataset directory contains the scene_xxxxx or environment_xxxxx folder/s")
        raise SystemExit
